# SEP-2106: Full JSON Schema 2020-12 Support for Tool Schemas
...
This demonstrates that the current restrictions create genuine ecosystem friction that SEP-834 would eliminate.
...
|-------------------|----------------|
| `type` must be string, not array | `type: "object"` required at root |
| Couldn't use standard null handling | Can't use `oneOf`/`anyOf` at root |
...
- **npm**: `@olaservo/mcp-sdk@1.25.2-sep834.3`
- **Key changes**:
  - `inputSchema`: Any valid JSON Schema object (compositions like `oneOf`/`anyOf` at root)
...
Three demo tools added to the `everything` server demonstrating SEP-834 capabilities:
...
- **npm**: `@olaservo/mcp-server-everything-sep834@1.1.0`
- **Tools**:
  - `get-weather-forecast`: Returns **raw array** of hourly forecasts directly in `structuredContent`
    - Matches the exact example from SEP-834's Motivation section
...
1. Update `inputSchema` types to allow any valid JSON Schema object (remove `type: "object"` constraint)
