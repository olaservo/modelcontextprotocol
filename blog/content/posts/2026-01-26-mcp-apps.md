...
// Send a follow-up message that triggers a model response
await app.sendMessage("User wants to drill down into Q4 results");

// Update model context silently (no response triggered)
await app.updateModelContext("User selected option B");

// Open external URLs (can't do this from a plain iframe)
await app.openLink("https://example.com/report");

// Log for debugging and telemetry
await app.sendLog("info", "Chart rendered with 150 data points");
```

A few key methods worth highlighting:

- [**`sendMessage`**](https://modelcontextprotocol.github.io/ext-apps/api/classes/App.html#sendMessage) sends a message to the host's chat interface. Use this when the app needs to add to the conversation or delegate back to the model.
- [**`updateModelContext`**](https://modelcontextprotocol.github.io/ext-apps/api/classes/App.html#updateModelContext) updates what the model knows without triggering a response. Use this for background state the model should be aware of.
- [**`openLink`**](https://modelcontextprotocol.github.io/ext-apps/api/classes/App.html#openLink) opens external URLs in a new tab. Sandboxed iframes can't do this on their own, so this is how apps link out.
- [**`sendLog`**](https://modelcontextprotocol.github.io/ext-apps/api/classes/App.html#sendLog) sends debug and telemetry data to the host for logging.

Since MCP Apps build on standard web primitives (HTML, JavaScript, `postMessage`), you can use any framework: React, Vue, Svelte, or vanilla JavaScript.
...
- **User consent**: Hosts can require explicit approval for UI-initiated tool calls and messages
