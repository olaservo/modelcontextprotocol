...
You could build a standalone web app and send users a link. However, MCP Apps offer three key advantages that a separate page can't match:
...
Traditional MCP tools return text or structured data that the host displays as part of the conversation. MCP Apps extend this pattern by allowing tools to declare a reference to an interactive UI in their tool description that the host renders in place.
...
2. **Resource fetch**: The host fetches the UI resource from the server. This resource contains a complete HTML page bundled with all its JavaScript and CSS.

3. **Sandboxed rendering**: Web hosts typically render the HTML inside a sandboxed [iframe](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe) within the conversation. The sandbox restricts the app's access to the parent page, ensuring security. The resource's `_meta.ui` object can include CSP (Content Security Policy) settings that further control the app's capabilities.
...
[**`updateModelContext(context)`**](https://modelcontextprotocol.github.io/ext-apps/api/classes/app.App.html#updatemodelcontext) updates the model's context with information from the app. This is a key differentiator from traditional iframes—your app can provide structured context that the LLM uses in subsequent responses, enabling tight integration between the UI and the conversation.
