...
MCP Apps enable servers to display interactive UI elements within conversational MCP clients. Instead of returning plain text, servers can return HTML interfaces (data visualizations, forms, dashboards) that render directly in the chat.
...
The `callServerTool` method sends a request through the host to the MCP server, executes the tool, and returns the result. Your app doesn't need to know the server's location or handle authentication. Keep in mind that each call involves a round-trip to the server, so design your UI to handle latency gracefully.
