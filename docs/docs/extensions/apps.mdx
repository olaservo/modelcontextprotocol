...
When an LLM decides to call a tool that supports MCP Apps, here's what happens:
...
3. **Sandboxed rendering**: The host renders the HTML inside a sandboxed iframe within the conversation. The sandbox restricts the app's access to the parent page, ensuring security.

4. **Bidirectional communication**: The app and host communicate through a postMessage-based protocol. The app can request tool calls, send messages, and receive data from the host.

This architecture means your app runs isolated from the host while still being able to interact with MCP server capabilities.
...
The fastest way to create an MCP App is using an AI coding assistant with the MCP Apps skill. If you prefer to set up a project manually, skip to [Manual setup](#manual-setup).

### Using an AI coding assistant

AI coding assistants with Skills support can scaffold a complete MCP App project for you. Skills are folders of instructions and resources that your assistant loads when relevant—they teach the AI how to perform specialized tasks like creating MCP Apps.

<Tabs>
  <Tab title="Claude Code">
If you have [Claude Code](https://docs.anthropic.com/en/docs/claude-code/overview) installed—whether in the terminal, [VS Code](https://docs.anthropic.com/en/docs/claude-code/ide-integrations/vscode), or [JetBrains IDEs](https://docs.anthropic.com/en/docs/claude-code/ide-integrations/jetbrains):

1. **Install the skill** by cloning the ext-apps repository:
   ```bash
   git clone https://github.com/modelcontextprotocol/ext-apps.git
   cp -r ext-apps/plugins/mcp-apps/skills/create-mcp-app ~/.claude/skills/
   ```

2. **Create your app** by asking Claude Code to build it:
   ```
   Create an MCP app that displays a color picker
   ```

   Claude Code will recognize the `create-mcp-app` skill is relevant, ask for your approval, then scaffold a complete project with server, UI, and configuration files.
...
If you use [GitHub Copilot in VS Code](https://code.visualstudio.com/docs/copilot/overview) with [Agent Skills](https://code.visualstudio.com/docs/copilot/customization/agent-skills) enabled:

1. **Install the skill** by cloning the ext-apps repository:
   ```bash
   git clone https://github.com/modelcontextprotocol/ext-apps.git
   cp -r ext-apps/plugins/mcp-apps/skills/create-mcp-app .github/skills/
   ```

2. **Create your app** by asking Copilot in agent mode to build it:
   ```
   Create an MCP app that displays a color picker
   ```

   Copilot will load the skill's instructions and scaffold a complete project with server, UI, and configuration files.
...
Learn more about [VS Code Agent Skills](https://code.visualstudio.com/docs/copilot/customization/agent-skills).
  </Tab>
</Tabs>
...
If you're not using an AI coding assistant, or prefer to understand the setup process, follow these steps.

#### Prerequisites

Before building MCP Apps, you should understand how MCP servers work, particularly [tools](/specification/2025-11-25/server/tools) and [resources](/specification/2025-11-25/server/resources). Familiarity with the [MCP TypeScript SDK](/docs/sdk) is helpful. You'll need Node.js 18 or higher.
...
MCP Apps is under active development. If you encounter issues or have ideas for improvements, open an issue on the [GitHub repository](https://github.com/modelcontextprotocol/ext-apps/issues). For broader discussions about the extension's direction, join the conversation in [GitHub Discussions](https://github.com/modelcontextprotocol/ext-apps/discussions) or the [SEP-1865 thread](https://github.com/modelcontextprotocol/specification/discussions/1865).
