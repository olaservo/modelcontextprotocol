...
offer three key advantages that a separate page can't match:
...
Traditional MCP tools return text or structured data that the host displays as
...
2. **Resource fetch**: The host fetches the UI resource from the server. This
   resource contains a complete HTML page bundled with all its JavaScript and CSS.
...
3. **Sandboxed rendering**: Web hosts typically render the HTML inside a
   ...
   page, ensuring security. The resource's `_meta.ui` object can include CSP
   (Content Security Policy) settings that further control the app's capabilities.
...
Clone the ext-apps repository:
    ...
    This list is not comprehensive. Other agents may support skills in different
    locations; check your agent's documentation.

    <Tip>
    
    In Claude Code, you can install the skill directly with `/plugin marketplace add modelcontextprotocol/ext-apps` and `/plugin install mcp-apps@modelcontextprotocol-ext-apps`.

    </Tip>

    Alternatively, you can use the [Vercel Skills CLI](https://vercel.com/changelog/introducing-skills-the-open-agent-skills-ecosystem)
    to install skills across different agents:

    ```bash
    npx skills add modelcontextprotocol/ext-apps
    ```

    For example, with Claude Code you can install the skill globally (available
    in all projects):
    ...
    Or install it for a single project only by copying to `.claude/skills/` in
  ...
  <Step title="Create your app">
    ...
    The agent will recognize the `create-mcp-app` skill is relevant, load its
    instructions, then scaffold a complete project with server, UI, and
    configuration files.
  ...
  <Step title="Test your app">
    Follow the instructions in [Testing your app](#testing-your-app) below. For
    the color picker example, start a new chat and ask Claude to provide you a
    color picker.
...
<Steps>
  <Step title="Create the project structure">
    ...
    The server registers the tool and serves the UI resource. The UI files get
    bundled into a single HTML file that the server returns when the host
  ...
  <Step title="Install dependencies">
    ...
    The `ext-apps` package provides helpers for both the server side (registering
    tools and resources) and the client side (the `App` class for UI-to-host
    communication). Vite with the `vite-plugin-singlefile` plugin bundles your
    UI into a single HTML file that can be served as a resource.
  ...
  <Step title="Configure the project">
    <Tabs>
      ...
      <Tab title="tsconfig.json">
        The TypeScript configuration targets modern JavaScript (`ES2022`) and
        uses ESNext modules with bundler resolution, which works well with Vite.
        The `include` array covers both the server code in the root and UI code
        in `src/`.
  ...
  <Step title="Build the project">
    With the project structure and configuration in place, continue to
    [Building an MCP App](#building-an-mcp-app) below to implement the server
    and UI.
  </Step>
</Steps>
...
- **`registerAppTool`**: Registers a tool with the `_meta.ui.resourceUri` field.
  When the host calls this tool, it receives the result along with the URI,
  telling it to fetch and render the UI.
...
[**`connect()`**](https://modelcontextprotocol.github.io/ext-apps/api/classes/app.App.html#connect)
establishes the connection to the host. Call this once when your app
initializes.

[**`callServerTool(params)`**](https://modelcontextprotocol.github.io/ext-apps/api/classes/app.App.html#callservertool)
invokes a tool on the MCP server and returns the result. The params object
should include `name` (the tool name) and `arguments` (the tool's input).

[**`ontoolresult`**](https://modelcontextprotocol.github.io/ext-apps/api/classes/app.App.html#ontoolresult)
is a callback that fires when the host pushes a tool result to your app. This
happens when the tool is first called and the host renders your UI.

[**`sendMessage(message)`**](https://modelcontextprotocol.github.io/ext-apps/api/classes/app.App.html#sendmessage)
sends an arbitrary message to the host. Hosts can use this for custom
integrations.

[**`sendLog(level, message)`**](https://modelcontextprotocol.github.io/ext-apps/api/classes/app.App.html#sendlog)
logs a message to the host's console, useful for debugging.

[**`sendOpenLink(url)`**](https://modelcontextprotocol.github.io/ext-apps/api/classes/app.App.html#sendopenlink)
requests that the host open a URL in the user's browser.

[**`updateModelContext(context)`**](https://modelcontextprotocol.github.io/ext-apps/api/classes/app.App.html#updatemodelcontext)
updates the model's context with information from the app. This is a key
differentiator from traditional iframes—your app can provide structured context
that the LLM uses in subsequent responses, enabling tight integration between
the UI and the conversation.
...
The `ext-apps` repository includes a test host for development. With the MCP server
running, run the following in a separate terminal:

```bash macOS/Linux
git clone https://github.com/modelcontextprotocol/ext-apps.git
cd ext-apps/examples/basic-host
npm install
export SERVERS='["http://localhost:3001"]'
npm start
```
...
cd ext-apps\examples\basic-host
npm install
$env:SERVERS='["http://localhost:3001"]'
npm start
```
...
MCP Apps are built on standard web primitives (HTML, JavaScript, and the
[postMessage API](https://developer.mozilla.org/docs/Web/API/Window/postMessage)),
so you can use any framework or none at all. The communication protocol is
JSON-RPC 2.0 over `postMessage`, meaning any technology that can send and
receive window messages will work.
...
[MCP Inspector](https://github.com/modelcontextprotocol/inspector). See the
[clients page](/clients) for the full list of MCP clients and their supported
features.

If you're building an MCP client and want to support MCP Apps, the SDK includes
an [**App Bridge**](https://modelcontextprotocol.github.io/ext-apps/api/modules/app-bridge.html)
module that handles rendering apps in sandboxed iframes, message passing, tool
call proxying, and security policy enforcement. See the
[API documentation](https://modelcontextprotocol.github.io/ext-apps/api/) for
