...
export const ClientsSection = ({ children }) => {
  useEffect(() => {
    filterStore.setState({ selectedCapabilities: [], searchText: '', visibleCount: 0, totalCount: 0 });
  }, []);
...
export const McpClient = ({ name, homepage, supports, sourceCode, instructions, children }) => {
  ...
  useEffect(() => {
    filterStore.setState({ totalCount: filterStore.state.totalCount + 1 });
    return () => filterStore.setState({ totalCount: filterStore.state.totalCount - 1 });
  }, []);

  useEffect(() => {
    if (isVisible) {
      filterStore.setState({ visibleCount: filterStore.state.visibleCount + 1 });
      return () => filterStore.setState({ visibleCount: filterStore.state.visibleCount - 1 });
    }
  }, [isVisible]);
