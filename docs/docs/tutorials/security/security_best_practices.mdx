...
- **DNS rebinding**: Domains that change DNS resolution between validation and use (e.g.,
  `https://attacker.com` resolving to a safe IP initially, then to `192.168.1.1`)
...
- Loopback: `127.0.0.0/8`, `::1` (except when explicitly allowed for development) (except when explicitly allowed for development)
