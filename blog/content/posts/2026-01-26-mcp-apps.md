...
the amazing work of [MCP-UI](https://github.com/MCP-UI-Org/mcp-ui) and the
...
await app.updateModelContext({ content: [{ type: "text", text: "User selected option B" }] });
```
...
If you're already building with MCP-UI, nothing breaks. The bridge and server patterns are
supported in ext-apps, and the MCP-UI client library is still the way third-party hosts,
such as Postman, Goose, and others, will support app rendering going forward. When you want
to move to the official extension, the migration is minimal.
...
For the first time, a MCP tool developer can ship an interactive experience that works
